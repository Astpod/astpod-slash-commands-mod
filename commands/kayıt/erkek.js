...
module.exports = class Erkek extends Command {
    constructor(client) {
        super(client, {
            name: "e",
    ...
    async run(client, interaction) {
        ...
        if(!member.roles.cache.has(cfg.erkek)) {
            setTimeout(() => {
                member.roles.add(cfg.erkek)          
         }, 1000);
        await member.roles.remove(cfg.unregister)
        }
        interaction.reply({content: `${cfg.ok} ${member} adlı kişi başarılı şekilde **Erkek** olarak kayıt edildi.`})

        Data.findOne({user: member.id}, async(err,res) => {
            if(!res) {
              let arr = []
              arr.push({isim: member.displayName, rol: cfg.erkek, yetkili: interaction.member.id})
              ...
              res.isimler.push({isim: member.displayName, rol: cfg.erkek, yetkili: interaction.member.id})
              res.save().catch(e => console.log(e))
            }
        })
        
        await Database.findOneAndUpdate({ guildID: interaction.guild.id, userID: interaction.user.id }, { $inc: { erkek: 1, toplam: 1 } }, { upsert: true });
